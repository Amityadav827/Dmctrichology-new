  ...
  return (
    <div className="border border-slate-200 rounded-2xl overflow-hidden bg-white">
      ...
      <div
        ...
        className="pp-cms-editor min-h-[420px] max-h-[640px] overflow-y-auto px-6 py-5 text-sm leading-relaxed text-slate-800 outline-none"
      />
      <style>{`
        .pp-cms-editor h2 { font-size: 22px; font-weight: 600; margin: 18px 0 8px; }
        .pp-cms-editor h3 { font-size: 18px; font-weight: 600; margin: 16px 0 6px; }
        .pp-cms-editor p { margin: 0 0 12px; }
        .pp-cms-editor ul, .pp-cms-editor ol { margin: 0 0 12px 22px; }
        .pp-cms-editor a { color: #2563eb; text-decoration: underline; }
      `}</style>
    </div>
  );
}
...
export default function PrivacyPolicyCMS() {
  const [data, setData] = useState({
    hero: { isEnabled: true, pageTitle: "Privacy Policy", breadcrumbLabel: "Privacy Policy", backgroundImage: "" },
    content: "",
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await axios.put("/privacy-policy", data);
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto bg-slate-50 min-h-screen">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-10">
        <div>
          <h1 className="text-2xl font-black text-slate-900 tracking-tight">Privacy Policy CMS</h1>
          <p className="text-sm text-slate-500 font-medium italic">Manage the Privacy Policy page content and SEO</p>
        </div>
        ...
        <div className="bg-white rounded-[32px] border border-slate-200 shadow-sm p-10">
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <textarea rows={8} value={data.seo?.schema || ""} onChange={e => setSeo("schema", e.target.value)} className={`${inputCls} font-mono`} placeholder='{"@context":"https://schema.org","@type":"WebPage","name":"Privacy Policy"}' />
            </div>
          </div>
        </div>
      )}
